// 参考URL：https://sweetalert2.github.io/#download
// 参考URL：https://mam-mam.net/javascript/game_slot.html
// JavaScriptの記述 START
function TMamSlot() {
    "use strict";
    this.d = [0, 0, 0]; //0～999
    ...
    this.draw = function () {
        ...
        for (let i = 0; i < this.d.length; i++) {
            for (let j = -4; j <= 4; j++) {
                ...
                let y =
                    (Math.sin((((j - pos / 100) / 4) * Math.PI) / 2) *
                        this.can.height) /
                    2;
                this.ctx.fillText(
                    v,
    ...
    this.starting = function () {
        for (let i = 0; i < this.d.length; i++) {
            if (this.ds[i] == 1) {
                ...
                if (this.dv[i] > 2) {
                    this.dv[i] -=
                        Math.floor((Math.random() * this.dv[i]) / 10) + 1;
                    if (this.dv[i] < 2) {
                        if (this.d[i] % 2 == 1) {
                            this.d[i]++;
                        }
        ...
        if (this.ds[0] == 0 && this.ds[1] == 0 && this.ds[2] == 0) {
            ...
            if (this.d[0] == 700 && this.d[1] == 700 && this.d[2] == 700) {
                ...
                Swal.fire({
                    ...
                    text: "また挑戦してね",
                    footer: '<a href="https://mam-mam.net/javascript/game_slot.html">このアプリの参考リンクはこちら</a>',
                });
            }
            this.draw();
        } else {
